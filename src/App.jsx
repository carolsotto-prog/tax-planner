...
export default function App() {
  ...
  async function checkPerfil(user) {
    ...
    const { data } = await supabase.from('profissionais').select('*').eq('email', user.email).single()
    setPerfil(data || null)
    setLoading(false)
  ...
  if (!perfil) return <Onboarding user={session.user} onComplete={() => checkPerfil(session.user)} />
  ...
  const nav = [
    ...
    { section: 'Gestão' },
    { id: 'clientes', icon: 'ti-building', label: 'Clientes & Projetos' },
    ...
    <div className="app-layout">
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          ...
          <button title="Sair" onClick={handleSignOut} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', padding: 4, flexShrink: 0 }}>
      ...
      <div className="main-area">
        ...
        <main className="page-content" style={{ padding: page === 'clientes' ? '16px' : '24px' }}>
          {pages[page]}
        </main>
      </div>
    </div>
